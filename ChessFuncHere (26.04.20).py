...
def undomove(chboard,boardhistlist,movehistlist): # Reverts board to 1 move earlier, removes 1 historical unit of board and hist list
    ...
    if size >= 1:
        if size == 1:
            chboard = cloneboard(startboard) # if only 1 move, revert board to starting board
        if size >= 2:
            chboard = cloneboard(boardhistlist[size - 1]) # revert board to previous board
        boardhistlist.pop(size - 1)
        movehistlist.pop(size - 1)
        return chboard, boardhistlist, movehistlist
    else:
        return chboard, boardhistlist, movehistlist