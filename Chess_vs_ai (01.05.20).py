...
def main():
    ...
    while True:
        ...
        if airun == 1:
            ...
            if ai_move == [999, 999, 999, 999]: ################################### ADD INTERFACE EVENT!! ###################################
                gamestatusbox()
                gamestatustext(chboard, ch.checkturn(chboard, boardhistlist, movehistlist), boardhistlist, movehistlist, checkmate)
                pg.time.delay(5000)
                return
            ...
            airun = 0
            if ch.stalematecheck(chboard, movehistlist,ch.checkturn(chboard, boardhistlist, movehistlist)) == True:  ###################### ADD INTERFACE EVENT!! #####################
                gamestatusbox()
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.MOUSEBUTTONDOWN and (fclick == 0 or fclick[0] == 999) and pg.mouse.get_pressed()[0] and airun == 0:
            ...
            elif (event.type == pg.MOUSEBUTTONDOWN) and (fclick != 0) and pg.mouse.get_pressed()[0] and (pawnpromote == 0) and airun == 0: # first click is in the board and on any piece
            ...
            elif (event.type == pg.MOUSEBUTTONDOWN) and (fclick != 0) and pg.mouse.get_pressed()[0] and pawnpromote == 1 and airun == 0:
...
def gamestatustext(chboard, turn, boardhistlist, movehistlist, checkmate, stalemate):
    ...
    if (ch.checkcheck(chboard, turn, movehistlist) == True) and (checkmate == False):
        ...
        SCREEN.blit(POKEFONT2.render('Stalemate', True, CRIMSON), (173, 607))
        SCREEN.blit(POKEFONT2.render('Stalemate', True, CRIMSON), (173, 607))
        return
...
def renderturnimage(turn): # render the icon that indicates the player's turn
    ...
    if turn == 1:
        SCREEN.blit(pg.transform.scale(whiteknight, (58, 55)), (57, 588))
    elif turn == 2:
