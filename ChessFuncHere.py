...
def p2pawncheckmove(chboard, pieceid, rank, file, movehistlist):  # determine if P1pawn can move or capture or NONE, returns True/False
    pawnlist, pawnpos, pawn1list, pawn1mcount = list(range(21, 29)), piecelocator(chboard, pieceid), list(
        range(1, 9)), 0  # pawn1mcount = player 1 pawn move-counter
    if pieceid in pawnlist:
        if (0 <= rank <= 7) and (0 <= file <= 7):
                ...
                if p1epcaptured[0] in pawn1list:  # condition: Must be player 1 pawn ####
                    ...
                    if pawn1mcount == 1:  # condition: Must be first move of player 1 pawn ####
                        if p1epcaptured[2] == 4:  # condition: Must be two square movement (rank 6 to rank 4)  ####
                            ...
                            if ((pieceidpos[1] - p1epcaptured[3]) == 1) or ((pieceidpos[1] - p1epcaptured[
                                3]) == -1):  # condition: player 1 pawn must be right beside player 2's pawn
                                if chboard[p1epcaptured[2] + 1][
                                    p1epcaptured[3]] == 0:  # condition: enpasse position must be unoccupied
                                    if ((p1epcaptured[2] + 1) == rank) and (p1epcaptured[
            ...
            for i in rdiagf:
                if i == [rank, file]:
                    if chboard[rank, file] not in piecetypelist(2, 'all'):  # CAPTURE
                        return [True, 'n']
            for i in ldiagf:
                if i == [rank, file]:
                    if chboard[rank, file] not in piecetypelist(2, 'all'):  # CAPTURE
...
def p1pawncheckmove(chboard, pieceid, rank, file, movehistlist):  # determine if P1pawn can move or capture or NONE, returns True/False
    ...
    if pieceid in pawnlist:
        if (0 <= rank <= 7) and (0 <= file <= 7):
                        ...
                        if pawn2mcount == 1:  # condition: Must be first move of player 2 pawn ####
                            if p2epcaptured[2] == 3:  # condition: Must be two square movement (rank 1 to rank 3)  ####
                                ...
                                if ((pieceidpos[1] - p2epcaptured[3]) == 1) or ((pieceidpos[1] - p2epcaptured[
                                    3]) == -1):  # condition: player 1 pawn must be right beside player 2's pawn
                                    if chboard[p2epcaptured[2] - 1][
                                        p2epcaptured[3]] == 0:  # condition: enpasse position must be unoccupied
            ...
            for i in rdiagf:
                if i == [rank, file]:
                    if chboard[rank, file] not in piecetypelist(1, 'all'):  # MOVE
                        return [True, 'n']
            for i in ldiagf:
                if i == [rank, file]:
                    if chboard[rank, file] not in piecetypelist(1, 'all'):  # MOVE
...
def pawncheckmove(chboard, turn, pieceid, rank, file, movehistlist):  # CORE PAWN MOVEMENT/CAPTURE FUNCTION, returns [True/False,p/n]
    if turn == 1:
...
def kingcheckmove(chboard, turn, pieceid, rank, file, movehistlist):  # CHECKS for 'check' & 'checkmate' conditions, determines validity of king movement/capture
    ...
    elif (kingprecheckmove(chboard, turn, pieceid, rank, file, movehistlist) == [True, 'ksc']) or (
            kingprecheckmove(chboard, turn, pieceid, rank, file, movehistlist) == [True,
                                                                                   'qsc']):  # Condition: valid rank/file movement index
        if turn == 1:  # P1 CASTLE
            for piece in p2pieces:
                if kingprecheckmove(chboard, turn, pieceid, rank, file, movehistlist) == [True,
        ...
        for piece in p1pieces:
            if pieceidentifier(staleboard, piece) == 10:  ##### P1 pawn #####
                ...
                for p1qrank in list(range(8)):
                    for p1qfile in list(range(8)):
                        ...
                        for rank in list(range(8)):
                            for file in list(range(8)):
                                if chboard(rank, file) != staleboard(rank,
                                                                     file):  # if queen is movable, stalemate false
                                    return False
                ...
                p1kicoordlist = [[piecepos[0] + 1, piecepos[1]], [piecepos[0] - 1, piecepos[1]],
                                 ...
                                 [piecepos[0] - 1, piecepos[1] + 1], [piecepos[0] - 1, piecepos[1] - 1]]
                if castlecheck(staleboard, turn, movehistlist) != ['n',
                                                                   'n']:  # if castling is possible, else nothing added to coordlist
    ...
    if turn == 2:  # P2 TURN #
        for piece in p2pieces:
            if pieceidentifier(staleboard, piece) == 20:  ##### P2 pawn #####
                piecepos = piecelocator(staleboard, piece)
                for p2prank in list(
                        range(piecepos[0] - 1, piecepos[0] - 3)):  # rank goes down by 2, all files for +0 and +1 rank
    ...
    if templistsize >= 100:
        for move in temphistlist:  # condition: no pawn movement
            ...
            if (move[1] == 'x') or (move[1] == 'ep'):  # condition: no piece captured
                return False
            if counter == 50:  # if no pawn movement or piece captures take place
                return True
    else:
        return False
...
def threefoldcheck(chboard, boardhistlist, movehistlist):  # determines if three-fold repetition rule draw can be declared by the player
    tfboard, tfcounter, tempboardlist, tfturnlist, tfmovelist, tfsubmovelist = np.zeros((8, 8)), 0, cloneboardlist(
        boardhistlist), [], [], []
    tfboardlist, tfsubboardlist = [], []
    eplist, tfepstatus = [], False  # tfepstatus determines if ep status stayed the same throughout
    castlelist, tfcastlestatus, p1idencastle, p2idencastle = [], False, 0, 0  # tfcastlestatus determines if ep status
    # stayed the same throughout
    for i in list(
            range(len(tempboardlist) + 1)):  # convert tempboardlist boards to uniformed pieceIDs for each piecetypes
        tempboardlist[i] = unipiecedboard(tempboardlist[i])
    tfboard = tempboardlist[len(tempboardlist) - 1]

    for board in tempboardlist:
        if numpy.array_equal(board, tfboard):
            tfcounter += 1
    ...
    if tfcounter >= 3:  # three identical positions (LATEST THREE)
        ...
        if len(turnlist) > 3:
            for removelatest in list(range(len(turnlist) - 3)):
                turnlist.pop(0)  # make turnlist have only three latest turns with same position
                        ...
                        if testboard[rank, file] in piecetypelist(1, 'pawn'):  # Search for P1 pawns
                            for p1rank in list(range(8)):
                                for p1file in list(range(8)):
                                    if pawncheckmove(testboard, 1, testboard[rank, file], p1rank, p1file, testhist) == [
                                        'True', 'ep']:  # EP PRESENT
                        ...
                        if testboard[rank, file] in piecetypelist(2, 'pawn'):  # Search for P2 pawns
                            for p2rank in list(range(8)):
                                for p2file in list(range(8)):
                                    if pawncheckmove(testboard, 2, testboard[rank, file], p2rank, p2file, testhist) == [
                                        'True', 'ep']:  # EP PRESENT
        ...
        for rank in list(range(8)):
            for file in list(range(8)):
                ...
                if tempboard[rank, file] in piecetypelist(1, 'pawn'):
                    if pawncheckmove(tempboard, 1, tempboard[rank, file], kingpos[0], kingpos[1], movehistlist)[
                        0] == True:
