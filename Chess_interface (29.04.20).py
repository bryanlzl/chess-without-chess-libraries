...
POKEFONThist = pg.font.Font("./media/8-Bit Madness.ttf", 24)
...
def main():
    ...
    while True:
        ...
        drawhistbox()
        drawhisttext(SCREEN, notationtext, [0, 0, 0], [575, 341, 245, 300], scroller)
...
def drawhisttext(SCREEN, text, color, rectangle, scroller):
    ...
    y = rectangle[1]
    lineSpacing = 1
    fontHeight = POKEFONThist.size("Tg")[1]
    while text:
        i = 1
        if y + fontHeight > rectangle[1] + rectangle[3]:
            break
        while POKEFONThist.size(text[:i])[0] < rectangle[2] and i < len(text):
            i += 1
        if i < len(text):
            i = text.rfind(" ", 0, i) + 1
        if scroller > 0:
            ...
            image = POKEFONThist.render(text[:i], True, color)
...
def drawhistbox():
    POKEFONT = pg.font.Font("./media/8-Bit Madness.ttf", 25)
    mainbox = pg.Rect(570, 300, 270, 261)
